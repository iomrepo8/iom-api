...
-- ADD NEW MODULE
IF NOT EXISTS(SELECT * FROM dbo.AspNetModules WHERE ModuleCode = 's-accounts-dashboard')
	INSERT INTO dbo.AspNetModules(ModuleCode, [Name], DateCreated, IsActive, [Order], ParentModuleCode, [SubModuleOrder])
	VALUES ('s-accounts-dashboard', 'Accounts Dashboard', GETUTCDATE(), 1, 7, 'm-accounts', 1)
GO
...
SET ModuleCode = 's-seat', ParentModuleCode = 'm-accounts', SubModuleOrder = 2
WHERE ModuleCode = 'm-seat'

UPDATE dbo.AspNetUserPermission SET ModuleCode = 's-seat'
WHERE ModuleCode = 'm-seat'

UPDATE [dbo].[RolePermissions] SET ModuleCode = 's-seat'
WHERE ModuleCode = 'm-seat'
