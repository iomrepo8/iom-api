...
-- ADD NEW MODULE
IF NOT EXISTS(SELECT * FROM dbo.AspNetModules WHERE ModuleCode = 'm-seat')
	INSERT INTO dbo.AspNetModules(ModuleCode, [Name], DateCreated, IsActive, [Order], [SubModuleOrder])
	VALUES ('m-seat', 'Seats', GETUTCDATE(), 1, NULL, 1)
GO
...
;WITH updateData AS (
	SELECT 
		...
		, CreatedBy = 'Darrel', DateCreated = GETDATE()
...
SELECT fd.RoleId, fd.ModuleId, 0, fd.ModuleCode, fd.CanView, fd.CanAdd, fd.CanEdit, fd.CanDelete, CreatedBy = 'Darrel', DateCreated = GETDATE() 
FROM updateData fd
