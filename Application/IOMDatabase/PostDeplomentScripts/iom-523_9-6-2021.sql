...
-- ADD NEW MODULE
IF NOT EXISTS(SELECT * FROM dbo.AspNetModules WHERE ModuleCode = 'm-tagging')
	INSERT INTO dbo.AspNetModules(ModuleCode, [Name], DateCreated, IsActive, [Order])
	VALUES ('m-tagging', 'Tagging', GETUTCDATE(), 1, 10)
GO
...
;WITH updateData AS (
	SELECT 
		...
		, CreatedBy = 'Jonathan', DateCreated = GETDATE()
	FROM AspNetRoles r, AspNetModules m
	WHERE r.RoleCode = 'SA' AND m.ModuleCode = 'm-tagging'
	UNION
	SELECT 
		RoleId = r.Id, ModuleId = m.Id, ModuleCode = m.ModuleCode,
		RoleName = r.Name, ModuleName = m.Name, r.RoleCode
		...
		, CreatedBy = 'Jonathan', DateCreated = GETDATE()
	FROM AspNetRoles r, AspNetModules m
	WHERE r.RoleCode = 'AM'	 AND m.ModuleCode = 'm-tagging'
	UNION
	SELECT 
		RoleId = r.Id, ModuleId = m.Id, ModuleCode = m.ModuleCode,
		RoleName = r.Name, ModuleName = m.Name, r.RoleCode
		...
		, CreatedBy = 'Jonathan', DateCreated = GETDATE()
	FROM AspNetRoles r, AspNetModules m
	WHERE r.RoleCode IN ('CL', 'LA', 'TM') AND m.ModuleCode = 'm-tagging'
	UNION
	SELECT 
		RoleId = r.Id, ModuleId = m.Id, ModuleCode = m.ModuleCode,
		RoleName = r.Name, ModuleName = m.Name, r.RoleCode
		...
		, CreatedBy = 'Jonathan', DateCreated = GETDATE()
	FROM AspNetRoles r, AspNetModules m
	WHERE r.RoleCode = 'AG' AND m.ModuleCode = 'm-tagging'
)
