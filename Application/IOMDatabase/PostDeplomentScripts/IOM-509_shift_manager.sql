...
-- ADD NEW MODULE
IF NOT EXISTS(SELECT * FROM dbo.AspNetModules WHERE ModuleCode = 's-shiftmanager')
	INSERT INTO dbo.AspNetModules(ModuleCode, [Name], DateCreated, IsActive, [Order], ParentModuleCode, [SubModuleOrder])
	VALUES ('s-shiftmanager', 'Shift Manager', GETUTCDATE(), 1, 7, 'm-timekeeps', 5)
GO
...
;WITH insertData AS (
    SELECT [Zone] = 'Philippine Standard Time', [Value] = 'Philippine Standard Time' UNION
	SELECT [Zone] = 'Eastern Standard Time', [Value] = 'Eastern Standard Time'
)
 
 INSERT INTO dbo.TimeZone([Zone], [Value])
 SELECT i.[Zone], i.[Value] FROM insertData i
 WHERE NOT EXISTS(SELECT 1 FROM dbo.TimeZone WHERE [Zone] = i.[Zone])
 GO